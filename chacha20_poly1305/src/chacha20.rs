...
impl ChaCha20 {
    ...
    pub fn apply_keystream(&mut self, buffer: &mut [u8]) {
        ...
        // falling through to the per-block scalar loop. The `if let Ok(...)`
        // matches unconditionally in practice — `chunks_exact_mut` guarantees
        // each yielded chunk is exactly `4 * CHACHA_BLOCKSIZE` bytes — but
        // avoids a visible panic path in `apply_keystream`.
        #[cfg(all(target_arch = "aarch64", target_endian = "little", target_feature = "neon"))]
