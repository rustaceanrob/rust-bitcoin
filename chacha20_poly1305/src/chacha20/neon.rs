...
//!
//! NEON is a required baseline feature of the aarch64 architecture, so no
//! runtime CPU-feature detection is used. The module is gated on little-endian
//! aarch64 only; the big-endian case falls back to the scalar path.
...
/// at `start_block`.
///
/// `chunk` must be exactly 256 bytes (4 × 64).
#[inline(always)]
pub(super) fn apply_4_blocks(chunk: &mut [u8], key: &Key, nonce: &Nonce, start_block: u32) {
    debug_assert_eq!(chunk.len(), 4 * 64);
    // SAFETY: NEON is guaranteed on aarch64; `chunk` is a valid mutable slice of
    // 256 bytes; the routine only reads and writes within that slice.
    unsafe { apply_4_blocks_impl(chunk, key, nonce, start_block) };
}

#[inline]
#[allow(clippy::too_many_lines)]
unsafe fn apply_4_blocks_impl(chunk: &mut [u8], key: &Key, nonce: &Nonce, start_block: u32) {
    // Load key and nonce as little-endian u32s.
    let k = load_key(key);
    let n = load_nonce(nonce);

    // Initial state, one broadcast register per state word.
    ...
    let init4 = aarch64::vdupq_n_u32(k[0]);
    let init5 = aarch64::vdupq_n_u32(k[1]);
    let init6 = aarch64::vdupq_n_u32(k[2]);
    let init7 = aarch64::vdupq_n_u32(k[3]);
    let init8 = aarch64::vdupq_n_u32(k[4]);
    let init9 = aarch64::vdupq_n_u32(k[5]);
    let init10 = aarch64::vdupq_n_u32(k[6]);
    let init11 = aarch64::vdupq_n_u32(k[7]);
    ...
    let init13 = aarch64::vdupq_n_u32(n[0]);
    let init14 = aarch64::vdupq_n_u32(n[1]);
    let init15 = aarch64::vdupq_n_u32(n[2]);
    ...
    xor_into(ptr, w0, w1, w2, w3); // bytes  0..16 of each block
    xor_into(ptr.add(16), w4, w5, w6, w7); // bytes 16..32
    xor_into(ptr.add(32), w8, w9, w10, w11); // bytes 32..48
    xor_into(ptr.add(48), w12, w13, w14, w15); // bytes 48..64
}
...
/// Rotate left by 12 within each 32-bit lane, via shift-or.
#[inline(always)]
unsafe fn rotl12(x: aarch64::uint32x4_t) -> aarch64::uint32x4_t {
    aarch64::vorrq_u32(aarch64::vshlq_n_u32::<12>(x), aarch64::vshrq_n_u32::<20>(x))
}
...
/// Rotate left by 7 within each 32-bit lane, via shift-or.
#[inline(always)]
unsafe fn rotl7(x: aarch64::uint32x4_t) -> aarch64::uint32x4_t {
    aarch64::vorrq_u32(aarch64::vshlq_n_u32::<7>(x), aarch64::vshrq_n_u32::<25>(x))
}

/// Transpose four "same-word-across-blocks" vectors into four "block words"
/// vectors and XOR each into the corresponding block's 16-byte slot at `dst`.
///
/// Given `v_i[j]` = word `i` of block `j`, this writes for each block `j`
/// the four consecutive words `[v_0[j], v_1[j], v_2[j], v_3[j]]` into
/// `dst[j * 64 .. j * 64 + 16]`, XOR'd against the existing contents.
#[inline(always)]
unsafe fn xor_into(
    dst: *mut u8,
    ...
    for (j, block) in blocks.iter().enumerate() {
        let block_dst = dst.add(j * 64);
        let existing = aarch64::vld1q_u8(block_dst);
        let block_bytes = aarch64::vreinterpretq_u8_u32(*block);
        let xored = aarch64::veorq_u8(existing, block_bytes);
        aarch64::vst1q_u8(block_dst, xored);
    }
}

#[inline(always)]
fn load_key(key: &Key) -> [u32; 8] {
    let mut out = [0u32; 8];
...
#[inline(always)]
fn load_nonce(nonce: &Nonce) -> [u32; 3] {
    let mut out = [0u32; 3];
    for (i, word) in out.iter_mut().enumerate() {
        *word = u32::from_le_bytes([
            nonce.0[i * 4],
