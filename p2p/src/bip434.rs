...
use encoding::{ByteVecDecoder, BytesEncoder, CompactSizeEncoder, Decoder2, Encoder2};
...
impl encoding::Encode for FeatureId {
    ...
    fn encoder(&self) -> Self::Encoder<'_> {
        FeatureIdEncoder::new(Encoder2::new(
            CompactSizeEncoder::new(self.feature.len()),
            BytesEncoder::without_length_prefix(self.feature.as_bytes()),
        ))
    }
}
...
crate::decoder_newtype! {
    /// Decoder for [`FeatureData`].
    #[derive(Debug, Default, Clone)]
    pub struct FeatureIdDecoder(FeatureIdInnerDecoder);

    fn map_push_bytes_err(err: <FeatureIdInnerDecoder as encoding::Decoder>::Error) -> FeatureIdDecoderError {
        FeatureIdDecoderError::Decoder(err)
    }

    fn end(
        result: Result<Vec<u8>, <FeatureIdInnerDecoder as encoding::Decoder>::Error>
    ) -> Result<FeatureId, FeatureIdDecoderError> {
        let feature_id = result.map_err(FeatureIdDecoderError::Decoder)?;
        let feature_string = String::from_utf8(feature_id).map_err(|_| FeatureIdDecoderError::Malformed(FeatureIdError::NotAscii))?;
...
impl encoding::Encode for FeatureData {
    ...
    fn encoder(&self) -> Self::Encoder<'_> {
        FeatureDataEncoder::new(Encoder2::new(
...
crate::decoder_newtype! {
    ...
    fn map_push_bytes_err(err: <FeatureDataInnerDecoder as encoding::Decoder>::Error) -> FeatureDataDecoderError {
        FeatureDataDecoderError::Decoder(err)
    }

    fn end(
        result: Result<Vec<u8>, <FeatureDataInnerDecoder as encoding::Decoder>::Error>
    ) -> Result<FeatureData, FeatureDataDecoderError> {
        let data = result.map_err(FeatureDataDecoderError::Decoder)?;
        FeatureData::new(data).map_err(FeatureDataDecoderError::Malformed)
    }
}
