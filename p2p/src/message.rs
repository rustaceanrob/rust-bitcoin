...
impl Decodable for RawNetworkMessage {
    fn consensus_decode_from_finite_reader<R: BufRead + ?Sized>(
        ...
        let payload = match &cmd.0[..] {
            ...
            "feefilter" => NetworkMessage::FeeFilter(FeeRate::from_sat_per_kvb(
                u64::consensus_decode_from_finite_reader(&mut mem_d)?.try_into().unwrap_or(u32::MAX),
            )),
...
impl Decodable for V2NetworkMessage {
    fn consensus_decode_from_finite_reader<R: BufRead + ?Sized>(
        ...
        let payload = match short_id {
            ...
            5u8 => NetworkMessage::FeeFilter(FeeRate::from_sat_per_kvb(
                u64::consensus_decode_from_finite_reader(r)?.try_into().unwrap_or(u32::MAX)
            )),
